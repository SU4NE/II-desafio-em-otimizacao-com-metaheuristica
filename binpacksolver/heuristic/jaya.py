...
from binpacksolver.utils import (check_end,
                                 fitness, generate_solution,best_fit_decreasing,
                                 theoretical_minimum, generate_initial_population, bestfit_population)

def repair_solution(solution: np.ndarray, new_solution: np.ndarray, c:int) -> np.ndarray:
    """
    ...
    remaining_values = [item for item, count in number_count_dict.items() for _ in range(count)]
    random.shuffle(remaining_values)
    solution = best_fit_decreasing(np.array(remaining_values), c, generate_solution(np.array(modify), c, VALID=True)[0])
...
def jaya_optimization(
    ...
    pop_bins, _ = generate_initial_population(array_base, c, population_size, juice=False, VALID=True)
    ...
    while check_end(th, best_fit, time_max, start, time.time(), max_it, it):
        ...
        candidate_positions = pop_matrix[:, :-1] + a_matrix * (best_solution - np.abs(pop_matrix[:, :-1])) \
        ...
        for i in range(candidate_positions.shape[0]):
            repaired_solution = repair_solution(pop_matrix[i, :-1].copy(), candidate_positions[i].copy(), c)
            candidate_positions[i] = repaired_solution
            
        fitness_values = np.apply_along_axis(lambda x: fitness(x, c), 1, candidate_positions)
        candidate_positions = np.hstack((candidate_positions, fitness_values[:, np.newaxis])).astype(int)
    ...
    best_idx = np.argmin(pop_matrix[:, -1])
    return generate_solution(pop_matrix[best_idx, :-1], c, VALID=True)[0], pop_matrix[best_idx, -1]